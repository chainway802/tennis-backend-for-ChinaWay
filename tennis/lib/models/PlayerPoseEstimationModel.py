...
class PlayerPoseEstimationModel(TRTModel):
    ...
    def _pre_process(self, frame, bbox, channel_convert=False, resized_shape=(320, 320), resolution=(1920, 1080)):
        ...
        x1, y1, x2, y2 = bbox
        x1, y1, x2, y2 = max(0, x1-10), max(0, y1-30), min(resolution[0], x2+10), min(resolution[1], y2+10)
        # 裁剪图片并进行预处理，推理
        img_crop = frame[int(y1):int(y2), int(x1):int(x2), :]
        cv2.resize(img_crop, resized_shape).astype(np.float32)
        np.transpose(img_crop, (2, 0, 1))
    ...
    def inference(self, frame, bbox):
        data_pre = self._pre_process(frame, bbox, channel_convert=False, resized_shape=self.inputshape)
        pose_results = super().inference(data_pre)
        kpts_post = self._post_process(pose_results, self.inputshape, frame.shape, bbox[:2])
        return kpts_post
    
    def _post_process(self, pose_results, resized_shape, img_shape, bbox_xy1):

...
def detect_court(frame):
    ...
    # 过滤出球场线
    filtered = _filter_pixels(gray)

    # 霍夫变换检测水平和垂直的直线
    horizontal_lines, vertical_lines = _detect_lines(filtered)

    # 获取实际球场线和参照球场线之间透视变换的变换矩阵
    pmatrix, inv_pmatrix, score = _find_homography(horizontal_lines, vertical_lines, gray)

    # 将参照球场变换后返回
    lines = _find_lines_location(pmatrix)

    return lines, pmatrix


def track_court(frame, pmatrix, cdist=None):
    """
    检测后跟踪球场线位置
    :param frame: 当前帧的图像
    :param pmatrix: 之前的透视变换矩阵
    :param cdist: 微调阈值
    :return: 球场线，透视变换矩阵
    """
    # 拷贝原始图像，获取灰度图
    ...
    # 初始化参数
    v_height, v_width = gray.shape
    dist = (5 if cdist is None else cdist)
    # 获取参考球场4个关键点映射到图像的点
    conf_points = np.array(COURT_CONFERENCE.court_conf[5], dtype=np.float32).reshape((-1, 1, 2))
    frame_points = cv2.perspectiveTransform(conf_points, pmatrix).squeeze().round()

    # 获取4个关键点对应的4条直线
    line1 = frame_points[:2]
    line2 = frame_points[2:4]
    line3 = frame_points[[0, 2]]
    line4 = frame_points[[1, 3]]
    ...
    for line in lines:
        ...
        p1 = None
        p2 = None
        if line[0][0] > v_width or line[0][0] < 0 or line[0][1] > v_height or line[0][1] < 0:
            for p in points_on_line:
                if 0 < p[0] < v_width and 0 < p[1] < v_height:
                    p1 = p
                    break
        if line[1][0] > v_width or line[1][0] < 0 or line[1][1] > v_height or line[1][1] < 0:
        ...
        for p in points_on_line:
            ...
            bottom_y, bottom_x = min(p[1] + dist, v_height), min(p[0] + dist, v_width)
        ...
        new_lines.append(((int(x - vx * v_width), int(y - vy * v_width)),
                          (int(x + vx * v_width), int(y + vy * v_width))))

        # 如果发现微调的采样点少于50个，则重新检测球场线，而不是跟踪球场线
        if len(new_points) < 50:
            if dist > 20:
                print("球场线变化较大，重新检测")
                return detect_court(frame)
            else:
                print("微调球场线失败，增大调整阈值")
                dist += 5
                return track_court(frame)

    # 从新线条中获得透视变换矩阵
    i1 = _line_intersection(new_lines[0], new_lines[2])
    i2 = _line_intersection(new_lines[0], new_lines[3])
    i3 = _line_intersection(new_lines[1], new_lines[2])
    i4 = _line_intersection(new_lines[1], new_lines[3])
    intersections = np.array([i1, i2, i3, i4], dtype=np.float32)
    pmatrix, _ = cv2.findHomography(np.float32(COURT_CONFERENCE.court_conf[5]), intersections, method=0)
...
def _filter_pixels(gray):
    ...
    """
    dist_tau = 3  # 控制线宽的参数
    intensity_threshold = 40  # 判断球场线的参数
...
def _detect_lines(gray):
    ...
    :return:
    """
    minLineLength = 100  # 最短线段长度
    maxLineGap = 20  # 被判别为同一直线的最大距离
    # 霍夫变换提取直线
    ...
    # 合并直线
    horizontal, vertical = _merge_lines(horizontal, vertical, gray.shape[0], gray.shape[1])
...
def _merge_lines(horizontal_lines, vertical_lines, v_height, v_width):
    """
    合并属于同一框架的直线
        ...
        if mask[i]:
            for j, s_line in enumerate(vertical_lines[i + 1:]):
                if mask[i + j + 1]:
                    x1, y1, x2, y2 = line
                    x3, y3, x4, y4 = s_line
                    xi, yi = _line_intersection(((x1, y1), (x2, y2)), ((xl, yl), (xr, yr)))
                    xj, yj = _line_intersection(((x3, y3), (x4, y4)), ((xl, yl), (xr, yr)))
...
def _find_homography(horizontal_lines, vertical_lines, gray):
    """
    ...
    for horizontal_pair in list(combinations(horizontal_lines, 2)):
        for vertical_pair in list(combinations(vertical_lines, 2)):
            ...
            v1, v2 = vertical_pair
            # 寻找所有直线的交点
            i1 = _line_intersection((tuple(h1[:2]), tuple(h1[2:])), (tuple(v1[0:2]), tuple(v1[2:])))
            i2 = _line_intersection((tuple(h1[:2]), tuple(h1[2:])), (tuple(v2[0:2]), tuple(v2[2:])))
            i3 = _line_intersection((tuple(h2[:2]), tuple(h2[2:])), (tuple(v1[0:2]), tuple(v1[2:])))
            i4 = _line_intersection((tuple(h2[:2]), tuple(h2[2:])), (tuple(v2[0:2]), tuple(v2[2:])))
            intersections = [i1, i2, i3, i4]
            intersections = _sort_intersection_points(intersections)

            # 寻找变换矩阵
            matrix, _ = cv2.findHomography(np.float32(COURT_CONFERENCE.court_conf[5]), np.float32(intersections), method=0)
            inv_matrix = cv2.invert(matrix)[1]
            # 获取变换得分
            confi_score = _get_confi_score(matrix, gray)
            # 维护得分最大的变换
...
def _sort_intersection_points(intersections):
    """
    将交叉点从左上角向右下角排序
    :param intersections:
    :return:
...
def _get_confi_score(matrix, gray):
    """
    计算转换得分
    :param matrix: 透视变换矩阵
    :param gray: 球场线灰度图
    :return: 得分
    """
    court = cv2.warpPerspective(COURT_CONFERENCE.court, matrix, gray.shape[::-1])
    court[court > 0] = 1
    gray = gray.copy()
    gray[gray > 0] = 1
    correct = court * gray
...
def _find_lines_location(pmatrix):
    """
    将标准球场线转换为图像中的球场线
    :param pmatrix: 透视变换矩阵
    :return: 图像中的球场线
    """
    conference_court_lines = np.array(COURT_CONFERENCE.get_conference_court_lines(), dtype=np.float32).reshape((-1, 1, 2))
    lines = cv2.perspectiveTransform(conference_court_lines, pmatrix).reshape(-1)
...
class CourtReference(object):
    ...
    def __init__(self):
        ...
        conference_court_image_path = os.path.join(os.path.dirname(__file__), "images/court_reference.png")
        self.court = cv2.cvtColor(cv2.imread(conference_court_image_path), cv2.COLOR_BGR2GRAY)

    def get_conference_court_lines(self):
        """
        获取能表示球场的参考球场线
        :return: 标准球场线
        """
